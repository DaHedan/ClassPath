...
2. **三种视图**：单日模式聚焦一天（左右滑动切换星期）、单周模式把所选周并排查看（可跳转任意周次）、本学期模式一览整学期安排。
...
2. 上课时间用**双圈表盘**选择，精确到分钟；**节次支持多选**，连续且其间无午/晚饭分隔的节次合并为一条上课时间（格子里连起来），其余按分段生成，每段时间可各自设置周次与地点。
3. **考试安排**：为课程登记考试日期、时间、地点与座位号，主页底部自动汇总、按时间排序。
4. 支持添加**备注**，记录老师联系方式、教材等额外信息。
5. **图片导入课程**（手机端）：从教务系统导出的课程表截图自动识别课程与上课时间，确认后一次性导入。
...
3. **分享内容可选**：导出 / 分享前可勾选是否带上用餐时间、调休安排、课程、教师、提前提醒、考试信息与备注。
