...
__想要了解关于课途的详细信息，请前往 [ClassPath v1.1 Wiki](https://github.com/DaHedan/ClassPath/wiki/ClassPath-v1.1-Wiki) 。__
...
如果你的需求是下载这个软件去使用，而不是需要源代码，请到 [Releases ClassPath v1.1](https://github.com/DaHedan/ClassPath/releases/tag/v1.1.0) 下载对应的安装包，无需下载源代码。
...
1. **多份课表**：可为不同学期创建多份课表，随时切换使用，新建后自动切换到新课表。
2. **三种视图**：单日模式聚焦当天（左右滑动切换星期）、单周模式并排查看指定一周（可跳转任意周次）、本学期模式一览整学期安排。
3. **自定义节次时间**：按楼宇配置每节的上课时间，午餐 / 晚餐时段按节次标注。
4. **节假日与调休**：自动接入国务院节假日安排，放假自动停课、调休补班日按设置日期排课。

### 课程管理

1. 课程卡片自定义：**颜色、编号、名称、教师、周次、地点**（楼宇 + 房号）。
2. 上课时间按**节次**（连续节次会连成一块显示）显示，同一门课可分多个时间段，各自单独设置周次与地点，顺序可拖动调整。
3. **考试安排**记录考试日期、时间、地点与座位号，主页底部自动汇总、按时间排序。
...
1. **二维码**：扫码 / 生成二维码快速导入导出课表；生成时可按需筛选分享内容（教师、提醒、考试、备注、用餐时间、调休等）并排除指定课程，手机端可调起系统分享面板发送给同学，也可直接保存到相册。
2. **文件备份**：导出 JSON 文件本地备份，随时恢复，跨平台格式一致。
3. **楼宇导入导出**：学校楼宇可单独导出与导入（可只导出楼宇，或附带各楼节次时间段），方便在多份课表之间复用。
4. **图片识别**：选择教务系统课表截图，本机离线识别并导入课程，**识别结果需手动核对**。
